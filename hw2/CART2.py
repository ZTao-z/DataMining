...
class CART_tree:
    ...
    def findFeatureAndThresParallel(self, feature, dataset):
        ...
        '''
        if shape(total_lost)[0] <= 2 * self.min_leaf_size:
            return
        index = np.argmin(total_lost[self.min_leaf_size : m - self.min_leaf_size], axis=0)
        if total_lost[self.min_leaf_size+index] < self.lowError:
            self.lowError = total_lost[self.min_leaf_size+index]
            self.bestFeature = feature
            #self.bestThres = thresList[0,self.min_leaf_size+index]
            self.bestThres = new_thresList[self.min_leaf_size+index]
        '''
...
class predictTreeThread:
    ...
    def run(self):
        ...
        for data in self.datas:
            self.result.append(self.ct.predict(self.tree, data))
        return self.result
        '''
